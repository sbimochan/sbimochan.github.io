...
var run = function () {
	...
	var result = github_user(function (data) {
		...
		if (typeof data.type !== 'undefined') {
			var resume = data.type === 'User' ? 'views/index.html' : 'views/resumeOrgs.html';
		} else {
			return false;
		}
		...
		function sortLanguages(languages, limit) {
			...
			for (var lang in languages) {
				...
				sorted_languages.push({
					...
					toString: function () {
						return (
							'<a href="https://github.com/search?q=user%3A' +
							username +
							'&l=' +
				...
				if (sorted.length > 0) {
					...
					$.each(sorted, function (index, repo) {
						...
						view = {
							...
							watchersLabel:
								repo.info.watchers === 0 || repo.info.watchers > 1 ? 'stars' : 'star',
					...
					if (data.length > 0) {
						$('#jobs')
							.html('')
							.append(
								'<p class="enlarge">All of this user\'s repositories seem to be forks. Sorry.</p>'
							);
					} else {
						$('#jobs')
							.html('')
							.append(
								'<p class="enlarge">Unfortunately, this user does not seem to have any <strong>public</strong> repositories.</p>'
							);
					}
				}
			},
